...
let trackIndex=cfg.track,track=TRACKS[trackIndex],finaleDuration=0,last=performance.now(),syncTimer=0,finaleStarted=false,sessionStartedAt=0,forceFinaleRequested=false;
...
async function begin(){await audio.init();engine.begin();audio.start(track);sessionStartedAt=performance.now();forceFinaleRequested=false;hint.style.opacity='0';finaleStarted=false;whiteout.classList.remove('show');}
async function input(x:number,y:number){if(engine.state==='idle'||engine.state==='rebirth'){advanceTrackIfNeeded();await begin();x=renderer.w/2;y=renderer.h/2;}if(engine.state!=='active'){engine.burst(x,y,3,.5);return;}const point=renderer.screenToWorld(x,y),result=judge(),note=audio.composeTap(point.x/renderer.w,point.y/renderer.h,result.synced);engine.tap(point.x,point.y,result);audio.tapFeedback(result.synced?result.accuracy:0,note);if(result.synced)renderer.focus(point.x,point.y,result.accuracy);}
...
function loop(now:number){const dt=Math.min(34,now-last);last=now;if(engine.state==='active'||engine.state==='finale')audio.setProgress(engine.progress);audio.updateComposition(now,dt,engine.sync.streak);engine.update(now,dt,judge,(result,index)=>{const n=engine.nodes[index];if(n)audio.composeContact(n.x/renderer.w,n.y/renderer.h,result.synced);audio.resonate(result.synced?result.accuracy:0,index);if(result.synced){showSync(result.label);if(n)renderer.focus(n.x,n.y,result.accuracy);}});const finaleReady=engine.energy>=engine.threshold&&engine.taps>=cfg.minimumTaps&&now-sessionStartedAt>=cfg.minimumActiveMs;if(engine.state==='active'&&(forceFinaleRequested||finaleReady)&&!finaleStarted)startFinale(now);finale.update(now,finaleDuration);whiteout.classList.toggle('show',engine.state==='whiteout');if(engine.state==='silence')audio.stop();if(engine.state==='rebirth'){hint.textContent='中央の光をタップ';hint.style.opacity='1';}if(engine.state==='idle'&&finaleStarted)advanceTrackIfNeeded();renderer.draw(now,engine,track);if(cfg.debug){const composition=audio.composition.state;debugReadout.textContent=`${engine.state} · ${track.name}\n${Math.round(audio.bpm)} BPM · energy ${Math.round(engine.energy)}/${engine.threshold}\ntaps ${engine.taps}/${cfg.minimumTaps} · density ${composition.tapDensity.toFixed(2)}/s · interval ${Math.round(composition.averageTapInterval)}ms\nmelody ${composition.currentMelodyNote} · [${composition.melodyNotes.join(', ')}]\narp [${composition.arpeggioNotes.join(', ')}] · hat ${composition.hiHatDensity.toFixed(2)}\nnext beat ${composition.nextQuantizedBeat} · SYNC ${composition.syncStreak}\nnodes ${engine.nodes.length}/10 · waves ${engine.waves.length}/20 · particles ${engine.particles.length}/${engine.sync.streak>=cfg.flowAt?280:220}\nwave a:${cfg.waveOpacity} w:${cfg.waveWidth} life:${cfg.waveLife}\nfinale ${cfg.finaleGather}/${cfg.finaleStructure}/${cfg.finaleExpand}ms`; }requestAnimationFrame(loop);}
if(cfg.debug){debugToggle.hidden=false;debugToggle.onclick=()=>debugPanel.hidden=!debugPanel.hidden;forceFinale.onclick=()=>{if(engine.state==='active')forceFinaleRequested=true;};}
