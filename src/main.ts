...
async function input(x:number,y:number){if(engine.state==='idle'||engine.state==='rebirth'){advanceTrackIfNeeded();await begin();x=renderer.w/2;y=renderer.h/2;}if(engine.state!=='active'){engine.burst(x,y,3,.5);return;}const result=judge();engine.tap(x,y,result);audio.tap(result.synced?result.accuracy:0,engine.taps);}
...
function loop(now:number){const dt=Math.min(34,now-last);last=now;if(engine.state==='active'||engine.state==='finale')audio.setProgress(engine.progress);engine.update(now,dt,judge,(result,index)=>{audio.resonate(result.synced?result.accuracy:0,index);if(result.synced)showSync(result.label);});
  ...
  if(engine.state==='rebirth'){hint.textContent='中央の光をタップ';hint.style.opacity='1';}
  if(engine.state==='idle'&&finaleStarted)advanceTrackIfNeeded();
