...
export type Node={id:number;x:number;y:number;note:number;life:number;pulse:number;lockedUntil:number;born:number;anticipation:number;hitFlash:number;hitDx:number;hitDy:number};
...
export class ResonanceEngine {
  ...
  tap(x:number,y:number,result:SyncResult,note=0){if(this.state!=='active')return;this.taps++;this.addNode(x,y,note);this.addWave(x,y,result.synced?1.65+(result.streak-1)*.12:1,0);this.burst(x,y,result.synced?16+(result.streak-1)*3:7,result.synced?3.1:1.4);this.energy=Math.min(this.threshold,this.energy+(result.synced?5+result.streak:2));this.trackTransition(result.streak);}
  private addNode(x:number,y:number,note:number){if(this.nodes.length>=DEFAULTS.maxNodes)this.nodes[0].life=0;this.nodes=this.nodes.filter(n=>n.life>0);this.nodes.push({id:++this.id,x,y,note,life:1,pulse:1,lockedUntil:0,born:performance.now(),anticipation:0,hitFlash:0,hitDx:0,hitDy:0});}
