export class Synth {
  ...
  tone(freq:number,duration=.12,type:OscillatorType='sine',gain=.05,when=this.ctx.currentTime,slide=0) {
    ...
    o.connect(g).connect(this.destination);this.activeVoices++;o.onended=()=>{this.activeVoices=Math.max(0,this.activeVoices-1);};o.start(when);o.stop(when+duration+.03);
  }
  noise(duration=.05,gain=.02,when=this.ctx.currentTime,cutoff=4800) {
    ...
    source.connect(filter).connect(g).connect(this.destination);this.activeVoices++;source.onended=()=>{this.activeVoices=Math.max(0,this.activeVoices-1);};source.start(when);
  }
}
